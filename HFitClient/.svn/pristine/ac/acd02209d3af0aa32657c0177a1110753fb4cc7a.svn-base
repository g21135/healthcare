...
public class ReviewMainController implements Initializable{

	
	public static int postNum = 0;
	
	@FXML ComboBox<String> searchSort;
	@FXML TableView<PostVO> reviewTable;
	...
	@FXML BorderPane reviewMainView;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			data = FXCollections.observableArrayList(post.allPostList(3));
		} catch (RemoteException e) {
		...
		reviewTable.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				...
				PostVO pv = reviewTable.getSelectionModel().getSelectedItem();
				postNum = reviewTable.getSelectionModel().getSelectedItem().getPost_no();
				changeScene("../post/ReviewPost.fxml"); 
				}
			}
		});
	}
	...
	public void tableSet(ObservableList<PostVO> data) {
		...
		pagination.setPageFactory(new Callback<Integer, Node>() {
			...
			public Node call(Integer pageIndex) {
				...
				reviewTable.setItems(getTableViewData(from, to));
				return reviewTable;
			}
		});
	}	
	...
	@FXML public void searchBtnClick() {
	...
	@FXML public void insertBtnClick(ActionEvent event) {
		FreePostWriteController.boardNum =3;
		changeScene("ReviewPostWriteMain.fxml"); 
	}
	...
	public FXMLLoader changeScene(String fxmlURL) {
		...
		reviewMainView.setCenter(parent);
		return loader;
	}
