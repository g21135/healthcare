...
public class QnAMainController implements Initializable{

	public static int postNum = 0;
	
	@FXML BorderPane qnaMainView;
	@FXML TableView<PostVO> qnaTable;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			data = FXCollections.observableArrayList(post.allPostList(4));
		} catch (RemoteException e) {
		...
		qnaTable.setOnMouseClicked(new EventHandler<MouseEvent>() {

			@Override
			public void handle(MouseEvent event) {
				if(qnaTable.getSelectionModel().isEmpty()) {
					return;
				}
				else {
				PostVO pv = qnaTable.getSelectionModel().getSelectedItem();
				postNum = qnaTable.getSelectionModel().getSelectedItem().getPost_no();
				changeScene("../post/QnaPost.fxml"); 
				}
			}
		});
	}
	...
	@FXML public void insertBtnClick(ActionEvent event) {
		FreePostWriteController.boardNum =4;
		changeScene("QnAPostWriteMain.fxml"); 
	}
