...
public class MatchingMainController implements Initializable {
	...
	@FXML
	TableColumn<sampleVO, Integer> btnClick;
	@FXML
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		for (MembershipVO vo : membershipList) {
			System.out.println(vo.getMem_id());
			System.out.println(vo.getMembership_startdate());
		}
		for (MatchVO vo : matchList) {
			System.out.println(vo.getMem_id());
		...
		for (int i = 0; i < memList.size(); i++) {
			mem_id = memList.get(i).getMem_id();

			for (int j = 0; j < membershipList.size(); j++) {
				if (mem_id.equals(membershipList.get(j).getMem_id())) {
					// mem_id =
				}
			}
		}
		...
		btnClick.setCellValueFactory(new PropertyValueFactory<>("btn"));
		btnClick.setStyle("-fx-alignment:center;");

		btnClick.setCellFactory(new Callback<TableColumn<sampleVO, Integer>, TableCell<sampleVO, Integer>>() {
			...
			public TableCell<sampleVO, Integer> call(TableColumn<sampleVO, Integer> param) {
				TableCell<sampleVO, Integer> cell = new TableCell<sampleVO, Integer>() {
					protected void updateItem(Integer item, boolean empty) {
						if (item != null) {
							Button btn = new Button("추가정보");
							String grade = sampleList.get(getIndex()).getMem_grade();
							if (grade.equals("골드")) {
								btn.setDisable(true);
							}
							btn.setOnAction(e -> {
								...
								UserHopeMain.mem_id = mem_id;
								UserHopeMain.name = name;
								modalDialog("../hope/UserHopeMain.fxml");
