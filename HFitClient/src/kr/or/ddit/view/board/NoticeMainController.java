...
public class NoticeMainController implements Initializable{
	
	public static int postNum = 0;
	
	@FXML TableColumn<PostVO, Integer> num;
	@FXML TableColumn<PostVO, String> title;
	@FXML TableColumn<PostVO, String> name;
	@FXML TableColumn<PostVO, Date> date;
	@FXML TableView<PostVO> noticeTable;
	@FXML BorderPane noticeMainView;
	@FXML Pagination pagination;
	@FXML TextField searchText;
	@FXML JFXButton searchBtn;
	@FXML ComboBox<String> searchSort;
	@FXML JFXButton noticeBoard;
	@FXML JFXButton event;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		noticeTable.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				if(noticeTable.getSelectionModel().isEmpty()) {
					return;
				}
				else {
				PostVO pv = noticeTable.getSelectionModel().getSelectedItem();
				postNum = noticeTable.getSelectionModel().getSelectedItem().getPost_no();
				changeScene("../post/NoticePost.fxml"); 
				}
			}
		});
	}
	...
	public void tableSet(ObservableList<PostVO> data) {
		...
		pagination.setPageFactory(new Callback<Integer, Node>() {
			...
			public Node call(Integer pageIndex) {
				...
				noticeTable.setItems(getTableViewData(from, to));
				return noticeTable;
			}
		});
	}	
	...
	@FXML public void insertBtnClick(ActionEvent event) {
		FreePostWriteController.boardNum = 1;
		changeScene("NoticePostWriteMain.fxml"); 
	}

	@FXML public void noticeBtnClick(ActionEvent event) {
		//event.setStyle("");
		noticeMainView.setTop(null);
		noticeBoard.setStyle("-fx-background-color: #FDF5E6;");		
		changeScene("NoticeMain.fxml");
	}

	@FXML public void eventBtnClick(ActionEvent event) {
		changeScene("EventMain.fxml"); 
	}
